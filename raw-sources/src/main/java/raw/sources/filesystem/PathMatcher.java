...
public interface PathMatcher {
     ...
     *
     * <p>Determines whether the pattern at least matches as far as the given base path goes,
     * assuming that a full path may then match as well.
     *
     * @param pattern the pattern to match against
     * @param path the path String to test
     * @return <code>true</code> if the supplied <code>path</code> matched, <code>false</code> if it
     ...
     *
     * <p>This method is supposed to find out which part of the path is matched dynamically through
     * an actual pattern, that is, it strips off a statically defined leading path from the given
     * full path, returning only the actually pattern-matched part of the path.
     *
     ...
     * pattern</code> parameter being a static path that wouldn't qualify as an actual {@link
     * #isPattern pattern}). A sophisticated implementation will differentiate between the static
