...
class Tree[T <: AnyRef with Product, +R <: T](val originalRoot: R, shape: TreeShape = LeaveAlone) {
   ...
   */
  def bottomupNoBridges(s: Strategy): Strategy = rule[Any] {
    case b: Bridge[_] => b
  } <+
   ...
   */
  def everywherebuNoBridges(s: Strategy): Strategy = bottomupNoBridges(attempt(s))
   ...
   */
  lazy val root =
    if (shape == EnsureTree) lazyclone(originalRoot, everywherebuNoBridges)
    else originalRoot
  ...
  lazy val child: TreeRelation[T] = {
    ...
    if (shape == CheckTree) {
      ...
      if (!msgBuilder.isEmpty) throw (new StructureIsNotATreeException(msgBuilder.result()))
    }
   ...
   */
  lazy val nodes: Vector[T] = child.domain
  ...
  def whenContains[V](t: T, v: => V): V =
    if (same(t, root) || (parent.containsInDomain(t))) v
    else throw new NodeNotInTreeException(t)
  ...
  def mapChild(f: Vector[T] => Vector[T]): TreeRelation[T] = {
    ...
    for (t <- child.domain) relation.set(t, f(child(t)))
    relation
  }
   ...
   */
  lazy val firstChild: TreeRelation[T] = mapChild(_.take(1))

  /**
   * A relation that relates a node to its last child.
   */
  lazy val lastChild: TreeRelation[T] = mapChild(_.takeRight(1))

  /**
   * The parent relation for this tree (inverse of child relation).
   */
  lazy val parent: TreeRelation[T] = child.inverse
  ...
  lazy val next: TreeRelation[T] = {
    ...
    for (t <- child.domain) {
      ...
      if (children.length > 0) {
        for (i <- 0 until children.length - 1) relation.set(children(i), Vector(children(i + 1)))
   ...
   * `next` relation.
   */
  lazy val prev: TreeRelation[T] = next.inverse
  ...
  lazy val sibling: TreeRelation[T] = {
    ...
    for (t <- parent.domain; p <- parent(t)) relation.set(t, child(p))
    relation
  }
   ...
   */
  def indexInVector(vector: Vector[T], t: T): Int = Comparison.indexOf(vector, t)

  /**
   * Return the first index of `t` in the children of `t's` parent node.
   * Counts from zero. Is zero for root.
   */
  def index(t: T): Int = parent(t) match {
    case Vector(p) => indexInVector(child(p), t)
    case _ =>
      if (same(t, root)) 0
      else throw new NodeNotInTreeException(t)
  }
   ...
   * Counts from zero. Is zero for root.
   */
  def indexFromEnd(t: T): Int = parent(t) match {
    case Vector(p) =>
      val c = child(p)
   ...
   */
  def isFirst(t: T): Boolean = prev(t).isEmpty

  /**
   * Return whether or not `t` is a last child. True for root.
   */
  def isLast(t: T): Boolean = next(t).isEmpty

  /**
   * Return whether or not `t` is the root of this tree.
   */
  def isRoot(t: T): Boolean = parent(t).isEmpty
