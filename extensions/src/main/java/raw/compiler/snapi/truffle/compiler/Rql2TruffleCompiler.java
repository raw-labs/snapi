...
public class Rql2TruffleCompiler extends Compiler
  ...
  public Either<scala.collection.immutable.List<ErrorMessage>, SourceProgram> parseAndValidate(
    ...
    buildInputTree(source, programContext)
        .right()
        .flatMap(
            tree -> {
                ...
                List<FunAppArg> programArgs =
                    Arrays.stream(args)
                        .map(
                            x -> {
                              Exp e;
                              if (x._2 instanceof ParamNull) {
                                e = NullConst.apply();
                              } else if (x._2 instanceof ParamByte) {
                                e =
                                    EnvironmentPackageBuilder.Parameter.apply(
                                        Rql2ByteType.apply(new HashSet<>()),
                                        StringConst.apply(x._1));

                                // TODO ALL CASES
                              } else {
                                throw new IllegalStateException("Unexpected value: " + x._2);
                              }
                              return FunAppArg.apply(
                                  e, mandatory.contains(x._1) ? Some.apply(x._1) : Option.empty());
                            })
                        .toList();
                return Right.apply(
                    Rql2Program.apply(
  ...
  private Value convertAnyToValue(Object v, Type t) {
    return null;
  }
