...
class PolyglotJsonWriter(os: OutputStream) extends Closeable {
  ...
  private var gen: JsonGenerator = _

  init()

  private def init(): Unit = {
    val factory = new JsonFactory()
    //factory.disable(JsonParser.Feature.AUTO_CLOSE_SOURCE) // Don't close file descriptors automatically on read
    gen = factory.createGenerator(os, JsonEncoding.UTF8)
  }
