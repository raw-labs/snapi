...
@TruffleTests class BinaryExpGtTest extends Rql2TruffleCompilerTestContext with CombinationSpecTestHelper {
  ...
  test("Time.Build(10, 30) > Time.Build(9, 30)")(it => it should evaluateTo("true"))
  test("Time.Build(9, 30) > 123")(it => it should typeErrorAs("time but got int"))

  test("Date.Build(2022, 1, 15) > Date.Build(2021, 2, 1)")(it => it should evaluateTo("true"))
  test("Date.Build(2022, 1, 15) > Timestamp.Build(2021, 2, 15, 0, 0)")(it => it should evaluateTo("true"))
  test("Date.Build(2010, 1, 15) > Date.Build(2022, 1, 15)")(it => it should evaluateTo("false"))
  test("Date.Build(2022, 1, 15) > Time.Build(10, 15)")(it =>
    it should typeErrorAs("expected compatible with date but got time")
  )

  test(""" "abc" > "abc"  """)(it => it should evaluateTo(""" false """))
  test(""" "bbc" > "abc"  """)(it => it should evaluateTo(""" true """))
  test(""" "abc" > "bbc"  """)(it => it should evaluateTo(""" false """))
  ...
  test("""Nullable.Build(2) > 1""")(_ should evaluateTo("true"))
  test("""Nullable.Build(1) > Nullable.Build(1)""")(_ should evaluateTo("false"))
  test("""Nullable.Build(2) > Nullable.Build(1)""")(_ should evaluateTo("true"))
  ...
  test("""let x: int = Success.Build(1) in 2 > x""")(_ should evaluateTo("true"))
  ...
  test("""let x: int = Success.Build(2), y: int = Success.Build(1) in x > y""")(_ should evaluateTo("true"))
  ...
  test(" number > number") { _ =>
    forAll(combinations(numbers, numbers)) {
      case (n1, n2) =>
        ...
        TestData(s"${n1.v2} > ${n2.v1}") should evaluateTo("true")
  ...
  test("non-number > non-number") { _ =>
    forAll(nonNumbers) { x =>
      ...
      TestData(s"${x.v2} > ${x.v1}") should evaluateTo("true")
    }
  }
  ...
  test("success comparable > comparable") { _ =>
    forAll(nonNumbers ++ numbers) { x =>
      TestData(s"""let x: ${x.tipe} = ${x.v2} in x > ${x.v1}""") should evaluateTo("true")
