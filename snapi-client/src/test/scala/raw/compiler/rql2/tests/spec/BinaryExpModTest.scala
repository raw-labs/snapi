...
@TruffleTests class BinaryExpModTest extends Rql2TruffleCompilerTestContext with CombinationSpecTestHelper {
  ...
  test("""2 % 3""".stripMargin) { it =>
    it should typeAs("int")
    it should evaluateTo("2")
  }

  test("""Decimal.From(5) % Decimal.From(3)""".stripMargin) { it =>
  ...
  case class TestNumber(tipe: String, value: String, expected: String, priority: Int)

  val numbers = Table(
    "numbers",
    TestValue("byte", "2b", "0b", 0),
    TestValue("short", "Short.From(2)", "Short.From(0)", 1),
    TestValue("int", "2", "0", 2),
    TestValue("long", "2l", "0l", 3),
    TestValue("float", "2f", "0f", 4),
    TestValue("double", "2d", "0d", 5),
    TestValue("decimal", "Decimal.From(2)", "Decimal.From(0)", 6)
  )
