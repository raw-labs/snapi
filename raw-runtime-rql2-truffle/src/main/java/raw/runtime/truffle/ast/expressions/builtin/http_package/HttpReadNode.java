...
public abstract class HttpReadNode extends ExpressionNode {
    ...
    protected ObjectTryable doRead(LocationObject locationObject,
        ...
        try {
            ...
            HttpByteStreamLocation location = builder.build(locationObject.getLocationDescription(), context.sourceContext());
            ...
            if (options.isDefined(statusListOption)) {
                ...
                if (Arrays.stream(statuses).noneMatch(status -> status == result.status())) {
                    String method = locationObject.getLocationDescription().getStringSetting("http-method").getOrElse(() -> "get");
                    return ObjectTryable.BuildFailure(String.format("HTTP %s failed, got %d, expected %s",
                            method.toUpperCase(),
                            result.status(),
                            String.join(",", Arrays.stream(statuses).mapToObj(Integer::toString).toArray(String[]::new))));
                }
            }
