...
public abstract class LocationDescribeNode extends ExpressionNode {
    ...
    @Specialization
    protected Object doDescribe(LocationObject locationObject, int sampleSize,
        ...
        try {
            ...
            if (descriptor instanceof ExcelInputFormatDescriptor) {
                ...
                SqlTableInputFormatDescriptor sqlTableDescriptor = (SqlTableInputFormatDescriptor) descriptor;
                format = "relational table";
                tipe = sqlTableDescriptor.tipe();
            } else if (descriptor instanceof SqlQueryInputFormatDescriptor) {
                SqlQueryInputFormatDescriptor sqlQueryDescriptor = (SqlQueryInputFormatDescriptor) descriptor;
                format = "relational query";
                tipe = sqlQueryDescriptor.tipe();
            } else if (descriptor instanceof TextInputStreamFormatDescriptor) {
                TextInputStreamFormatDescriptor textInputQueryDescriptor = (TextInputStreamFormatDescriptor) descriptor;
                comment = String.format("encoding %s (confidence: %s", textInputQueryDescriptor.encoding(), textInputQueryDescriptor.confidence()) + "%)";
                if (textInputQueryDescriptor.format() instanceof CsvInputFormatDescriptor) {
                    CsvInputFormatDescriptor csvDescriptor = (CsvInputFormatDescriptor) textInputQueryDescriptor.format();
                    ...
                    properties.put("nulls", nls.stream().map(x -> '"' + x + '"').collect(Collectors.joining(",", "[", "]")));

                    ArrayList<String> nans = new ArrayList<>();
                    csvDescriptor.nans().foreach(nans::add);
                    properties.put("nans", nans.stream().map(x -> '"' + x + '"').collect(Collectors.joining(",", "[", "]")));
                    ...
                    JsonInputFormatDescriptor jsonDescriptor = (JsonInputFormatDescriptor) textInputQueryDescriptor.format();
                    format = "json";
                    ...
                    HjsonInputFormatDescriptor hjsonDescriptor = (HjsonInputFormatDescriptor) textInputQueryDescriptor.format();
                    format = "hjson";
                    ...
                    XmlInputFormatDescriptor xmlDescriptor = (XmlInputFormatDescriptor) textInputQueryDescriptor.format();
                    format = "xml";
                    ...
                    LinesInputFormatDescriptor linesDescriptor = (LinesInputFormatDescriptor) textInputQueryDescriptor.format();
                    format = "lines";
            ...
            Rql2Type rql2Type = (Rql2Type) Rql2TypeUtils$.MODULE$.inferTypeToRql2Type(tipe, sampled, sampled);
            ...
            if (flatten instanceof Rql2RecordType) {
                ...
                if (flatten instanceof Rql2TypeWithProperties) {
                    ...
                    isNullable = ((Rql2TypeWithProperties) flatten).props().contains(Rql2IsNullableTypeProperty.apply());
                } else {
                    typeStr = SourcePrettyPrinter$.MODULE$.format(flatten);
                }
                RecordObject column = RawLanguage.get(this).createRecord();
