...
public abstract class TimestampTimeBucketIntervalNode extends ExpressionNode {
    @Specialization
    protected TimestampObject fromUnixTimestamp(IntervalObject intervalObj, TimestampObject timestampObj) {

        LocalDateTime timestamp = timestampObj.getTimestamp();
        LocalDateTime result;

        if (intervalObj.toMillis() == 0) throw new RawTruffleRuntimeException("interval cannot be empty in time_bucket", this);
        ...
        if (intervalObj.getWeeks() == 0 &&
            ...
            // the interval is only months and years we can perform the calculation on months, no need for approximation
