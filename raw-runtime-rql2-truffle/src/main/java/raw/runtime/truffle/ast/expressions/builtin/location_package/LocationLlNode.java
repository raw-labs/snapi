            ...
            for (int i = 0; i < size; i++) {
                ...
                if (values.apply(i)._2 instanceof DirectoryMetadata) {
                                ...
                                new ObjectOption(
                                        new TimestampObject(
                                                LocalDateTime.ofInstant(
                                                        directoryMetadata.modifiedInstant().get(),
                                                        ZoneOffset.UTC))));
                    } else {
                    ...
                    if (fileMetadata.modifiedInstant().isDefined()) {
                        records.writeMember(
                                ...
                                new ObjectOption(
                                        new TimestampObject(
                                                LocalDateTime.ofInstant(
                                                        fileMetadata.modifiedInstant().get(),
                                                        ZoneOffset.UTC))));
                    } else {
                        records.writeMember(metadata, "modified", new ObjectOption());
                    }
                    if (fileMetadata.size().isDefined()) {
                        records.writeMember(
                                metadata, "size", new LongOption((long) fileMetadata.size().get()));
                    } else {
                        records.writeMember(metadata, "size", new LongOption());
                    }
                    ...
                    for (int j = 0; j < blocksSize; j++) {
                        ...
                        records.writeMember(
                                block, "hosts", new StringList(fileMetadata.blocks()[j].hosts()));
