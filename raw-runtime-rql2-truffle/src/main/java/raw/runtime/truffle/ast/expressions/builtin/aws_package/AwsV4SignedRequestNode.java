        ...
        try {
            ...
            for (int i = 0; i < urlParamsLists.size(urlParamsSorted); i++) {
                canonicalQueryBuilder
                        .append(
                                URLEncoder.encode(
                                        (String)
                                                records.readMember(
                                                        urlParamsLists.get(urlParamsSorted, i),
                                                        "_1"),
                ...
                urlParamsVec.$plus$eq(
                        new Tuple2<>(
                                (String)
                                        records.readMember(
                                                urlParamsLists.get(urlParamsSorted, i), "_1"),
                                (String)
                                        records.readMember(
                                                urlParamsLists.get(urlParamsSorted, i), "_2")));
            }
            // remove last '&'
            ...
            for (int i = 0; i < headersLists.size(sortedHeaders); i++) {
                canonicalHeadersBuilder
                        .append(
                                ((String)
                                                records.readMember(
                                                        headersLists.get(sortedHeaders, i), "_1"))
                                        .toLowerCase())
                        .append(":")
                        .append(
                                (String)
                                        records.readMember(
                                                headersLists.get(sortedHeaders, i), "_2"))
                        .append("\n");
            ...
            for (int i = 0; i < headersLists.size(headers); i++) {
                headersParamsVec.$plus$eq(
                        new Tuple2<>(
                                ((String) records.readMember(headersLists.get(headers, i), "_1"))
                                        .toLowerCase(),
            ...
            String payloadHash =
                    toHexString(
                            getSha256Digest().digest(bodyString.getBytes(StandardCharsets.UTF_8)));
            ...
            map =
                    map.$plus(
                            Tuple2.apply(
                                    new LocationSettingKey("http-method"),
                                    new LocationStringSetting(method)));
            map =
                    map.$plus(
                            Tuple2.apply(
