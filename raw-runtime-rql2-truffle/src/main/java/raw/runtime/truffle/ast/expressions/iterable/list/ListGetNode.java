...
public abstract class ListGetNode extends ExpressionNode {
    ...
    protected Object listGetTryable(Object list, int index, @CachedLibrary("list") ListLibrary lists, @CachedLibrary("lists.get(list, index)") TryableLibrary triables) {
        return lists.get(list, index);
    }

    @Specialization(limit = "3", guards = {"lists.isElementReadable(list, index)"})
    protected Object listGetTryable(Object list, int index,
