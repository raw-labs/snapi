...
public final class IntervalObject implements TruffleObject {
    ...
    private final Pattern pattern = Pattern.compile("^P(?:(\\d+)Y)?(?:(\\d+)M)?(?:(\\d+)W)?(?:(\\d+)D)?(?:T(?:(\\d+)H)?(?:(\\d+)M)?(?:(\\d+)(?:\\.(\\d{1,3}))?S)?)?$");

    public IntervalObject(int years, int months, int weeks, int days, int hours, int minutes, int seconds, int millis) {
        this.years = years;
        this.months = months;
        this.weeks = weeks;
        this.days = days;
    ...
    public IntervalObject(String interval) {
        ...
        if (matcher.matches()) {
            ...
            throw new RawTruffleRuntimeException(String.format("could not parse interval from string '%s'", interval));
        }
...
//                millis
//        )
//            case _ => throw new RawRuntimeException(s"could not parse interval from string '$str'")
//        }


    public static IntervalObject normalize(int years, int months, int weeks, int days, int hours, int minutes, int seconds, int millis) {
        long totalMonths = 12 * (long) years + (long) months;
        long totalDays = 7 * (long) weeks + (long) days;

        long totalMillis = 24 * 3600000 * totalDays + 3600000L * hours + 60000L * minutes + 1000L * seconds + millis;
    ...
    public long toMillis() {
        ...
        return 24 * 3600000 * (long) totalDays + 3600000 * (long) hours + 60000 * (long) minutes + 1000 * (long) seconds + millis;
    }
