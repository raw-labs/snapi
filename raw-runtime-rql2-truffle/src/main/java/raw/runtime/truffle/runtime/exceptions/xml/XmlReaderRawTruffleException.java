...
public class XmlReaderRawTruffleException extends RawTruffleRuntimeException {
    ...
    public XmlReaderRawTruffleException(
            String message, RawTruffleCharStream stream, Node location) {
        super(createMessage(message, stream, null), null, location);
    }

    public XmlReaderRawTruffleException(
            Throwable cause, RawTruffleCharStream stream, Node location) {
        super(createMessage(cause.getMessage(), stream, null), cause, location);
    }

    public XmlReaderRawTruffleException(
            Throwable cause,
            RawTruffleXmlParser parser,
    ...
    private static String createMessage(
        ...
        if (parser != null) {
            builder.append(
                    String.format(
                            " (line %d column %d)", parser.currentLine(), parser.currentColumn()));
        }
