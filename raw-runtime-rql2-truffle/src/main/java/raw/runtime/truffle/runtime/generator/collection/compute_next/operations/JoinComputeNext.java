...
public class JoinComputeNext {
    ...
    private final NullableTryableLibrary nullableTryables = NullableTryableLibrary.getFactory().createDispatched(1);

    public JoinComputeNext(Object leftIterable, Object rightIterable, Closure remap, Closure predicate, Boolean reshapeBeforePredicate, Rql2TypeWithProperties rightRowType, RuntimeContext context, RawLanguage language) {
    ...
    @ExportMessage
    void init(@CachedLibrary(limit = "5") IterableLibrary iterables,
    ...
    private Object check(Object leftRow, Object rightRow) {
        ...
        if (reshapeBeforePredicate) {
            ...
            pass = nullableTryables.handleOptionTriablePredicate(nullableTryable,
                predicate.call(row),
                false);
            if (!pass) row = null;
        } else {
            pass = nullableTryables.handleOptionTriablePredicate(nullableTryable,
                predicate.call(leftRow, rightRow),
                false);
