...
import raw.runtime.truffle.ast.csv.reader.CsvParserOperations;
...
public class CsvReadFromStringComputeNext {
    ...
    public CsvReadFromStringComputeNext(String str, DirectCallNode rowParser, RawTruffleCsvParserSettings settings) {
    ...
    @ExportMessage
    void init(@Cached("create()") CsvParserOperations.InitCsvParserNode initParser,
              @Cached.Shared("closeParser") @Cached("create()") CsvParserOperations.CloseCsvParserNode closeParser) {
        try {
            parser = initParser.execute(stream, settings);
        } catch (CsvReaderRawTruffleException ex) {
            CsvReaderRawTruffleException newEx = new CsvReaderRawTruffleException(ex.getMessage(), null, stream);
    ...
    @ExportMessage
    void close(@Cached.Shared("closeParser") @Cached("create()") CsvParserOperations.CloseCsvParserNode closeParser) {
        closeParser.execute(parser);
    }
