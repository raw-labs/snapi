...
class OffHeapGroupByKey {
    private final TreeMap<Object, ArrayList<Object>> memMap; // in-memory map that's used as long as the data fits in memory.
    ...
    public OffHeapGroupByKey(Comparator<Object> keyCompare, Rql2TypeWithProperties kType, Rql2TypeWithProperties rowType,
                             RawLanguage language, RuntimeContext context, GroupByRecordShaper reshape) {
    ...
    // * if data were small and nothing was spilled to disk, return a simple in-memory one over the in-memory map.
    ...
    class GroupBySpilledFilesGenerator {
        ...
        private class InputBuffer {
            ...
            // extract a row from the current buffer. When reaching the end of the buffer, the key is set to null
        ...
        @ExportMessage
        void init() {
            // turn the list of spilled files into a list of Kryo buffers. Buffers will be read and dropped from
