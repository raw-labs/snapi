...
/**
 * An ordered map with multiple keys, that spills to disk when it reaches a certain size. Similar to OffHeapGroupByKey,
 * but supports multiple keys. Also the iterator it provides isn't returning the <key/value> pairs like OffHeapGroupByKey.
 * It's an iterator over the values (to give the effect of the OrderBy).
 */

public class OffHeapGroupByKeys {
    private final TreeMap<Object[], ArrayList<Object>> memMap; // in-memory map from arrays of keys to array of rows.
    private final ArrayList<File> spilledBuffers = new ArrayList<>(); // list of files that contain the spilled data.
    private final long maxSize; // maximum size of a spilled file.
    private int size; // estimated size of currently memory held objects (when reaching blockSize, spill to disk).
    ...
    private final int kryoOutputBufferSize, kryoInputBufferSize; // size of the kryo buffers used to write and read the data.
    ...
    public OffHeapGroupByKeys(Comparator<Object[]> keyCompare, Rql2TypeWithProperties[] kTypes, Rql2TypeWithProperties rowType, RawLanguage language,
        ...
        this.kryoOutputBufferSize = (int) context.settings().getMemorySize("raw.runtime.kryo.output-buffer-size");
        this.kryoInputBufferSize = (int) context.settings().getMemorySize("raw.runtime.kryo.input-buffer-size");
    ...
    class OrderBySpilledFilesGenerator {
        ...
        private ArrayList<InputBuffer> inputBuffers; // list of Kryo buffers that contain the spilled data.
        ...
        void init() {
            ...
            spilledBuffers.forEach(file -> {
                try {
                    inputBuffers.add(new InputBuffer(new UnsafeInput(new FileInputStream(file), kryoInputBufferSize)));
                } catch (FileNotFoundException e) {
                    throw new RawTruffleRuntimeException(e.getMessage());
                }
            });
        ...
        // Computes the next key to return. This is the "smallest key" (according to the compare function) among the keys
        ...
        public Object next() {
            if (currentKryoBuffer == null) {
                ...
                // Take note of the number of items stored in each in order to allocate the right amount of memory.
