...
public class NullableTryableHandler {
     ...
     * @return either returns a value extracted from nullable - tryable, or null for null option, or throws an error
     */
    public static Object handleOptionTriableUnsafe(Object maybeOptionTryable, Rql2TypeWithProperties typeWithProps) {
    ...
    /**
     * Handles both tryable and option predicates. If the tryable is failure, or the option is null, it returns the default value (something similar to getOrElse in scala)
     *
     * @param maybeOptionTryable An object value that can be either Option, Tryable or Both
     ...
     */
    public static Boolean handleOptionTriablePredicate(Object maybeOptionTryable, Rql2Type type, Boolean defaultValue) {
    ...
    /**
     * Handles both tryable and option. If the tryable is failure, it returns the error message as string value
     *
     * @param maybeOptionTryable An object value that can be either Option, Tryable or Both
     * @param typeWithProps      Rql2Type to extract nullable or tryable properties
     * @return Either the result value or null or the error message string
     */
    public static Object handleOptionTriable(Object maybeOptionTryable, Rql2TypeWithProperties typeWithProps) {
        if (typeWithProps.props().contains(Rql2IsTryableTypeProperty.apply())) {
            TryableLibrary triables = TryableLibrary.getFactory().create(maybeOptionTryable);
            if (triables.isSuccess(maybeOptionTryable)) {
                Object success = triables.success(maybeOptionTryable);
    ...
    private static Boolean handleOptionPredicate(Object value, Rql2TypeWithProperties typeWithProps, Boolean defaultValue) {
        if (typeWithProps.props().contains(Rql2IsNullableTypeProperty.apply())) {
            OptionLibrary options = OptionLibrary.getFactory().create(value);
            if (options.isDefined(value)) {
