...
public class LocationFromHttpNode extends ExpressionNode {
    ...
    public LocationFromHttpNode(String method, ExpressionNode url, ExpressionNode bodyString, ExpressionNode bodyBinary, ExpressionNode authCredentialName, ExpressionNode username, ExpressionNode password, ExpressionNode args, ExpressionNode headers, ExpressionNode expectedStatus) {
        this.method = method;
        this.url = url;
    ...
    public Object executeGeneric(VirtualFrame frame) {
        ...
        // FIXME (msb): Handle authCredentialName!!!!
        ...
        if (args != null) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (key != NullObject.INSTANCE && val != NullObject.INSTANCE) {
                    argsBuilder = (ArrayBuilder<Tuple2<String, String>>) argsBuilder.$plus$eq(Tuple2.apply((String) key, (String) val));
                }
            }
        }
        ...
        if (username != null && password != null) {
            ...
            argsBuilder = (ArrayBuilder<Tuple2<String, String>>) argsBuilder.$plus$eq(Tuple2.apply("Authorization", "Basic " + Base64.getEncoder().encodeToString((username + ":" + password).getBytes())));
        }
        ...
        if (this.headers != null) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                // ignore entries where key or val is null
                if (key != NullObject.INSTANCE && val != NullObject.INSTANCE) {
                    headersBuilder = (ArrayBuilder<Tuple2<String, String>>) headersBuilder.$plus$eq(Tuple2.apply((String) key, (String) val));
                }
            }
        }
        ...
        HttpByteStreamLocation location = new HttpByteStreamLocation(method, url, (Tuple2<String, String>[]) argsBuilder.result(), (Tuple2<String, String>[]) headersBuilder.result(), maybeBody, expectedStatusArray, rawSettings);
