...
public final class RawLanguage extends TruffleLanguage<RawContext> {
  ...
  protected CallTarget parse(ParsingRequest request) throws Exception {
    ...
    try {
      TreeWithPositions tree = new TreeWithPositions(source, false, true, programContext);
      if (tree.valid()) {
      ...
      if (e.getCause() instanceof CompilerParserException) {
        ...
        throw e; // rethrow if it's a different runtime exception
      }
    }
...
//    fix this
//      then go back tp snapi-client and try to get it build
//        hopefully interop types work now
//      but finally there's the question of where to put tests
//        for now move them from raw-extensions ONTO snapi-client and see if it works (FOR NOW)
//        MERGE BOTH THOSE build.sbt files
//
