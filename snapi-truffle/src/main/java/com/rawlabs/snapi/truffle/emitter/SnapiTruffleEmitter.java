...
import com.rawlabs.snapi.frontend.snapi.source.IdnExp;
import com.rawlabs.snapi.frontend.snapi.source.SourceNode;
...
import com.rawlabs.snapi.truffle.ast.controlflow.IfThenElseNode;
import com.rawlabs.snapi.truffle.ast.expressions.binary.*;
import com.rawlabs.snapi.truffle.ast.expressions.binary.DivNodeGen;
import com.rawlabs.snapi.truffle.ast.expressions.binary.ModNodeGen;
import com.rawlabs.snapi.truffle.ast.expressions.binary.MultNodeGen;
import com.rawlabs.snapi.truffle.ast.expressions.binary.SubNodeGen;
import com.rawlabs.snapi.truffle.ast.expressions.function.*;
...
import com.rawlabs.snapi.truffle.ast.expressions.unary.NegNodeGen;
import com.rawlabs.snapi.truffle.ast.expressions.unary.NotNodeGen;
import com.rawlabs.snapi.truffle.ast.local.*;
import com.rawlabs.snapi.truffle.ast.local.ReadClosureVariableNodeGen;
import com.rawlabs.snapi.truffle.ast.local.ReadLocalVariableNodeGen;
import com.rawlabs.snapi.truffle.ast.local.WriteLocalVariableNodeGen;
  ...
  private StatementNode recurseLetDecl(LetDecl ld) {
    return switch (ld) {
        ...
        int slot =
            switch (snapiType) {
              case SnapiUndefinedType ignored ->
                  getFrameDescriptorBuilder()
                      .addSlot(FrameSlotKind.Object, getIdnName(entity), null);
              ...
              case SnapiFloatType ignored ->
                  getFrameDescriptorBuilder()
                      .addSlot(FrameSlotKind.Float, getIdnName(entity), null);
              case SnapiDoubleType ignored ->
                  getFrameDescriptorBuilder()
                      .addSlot(FrameSlotKind.Double, getIdnName(entity), null);
              case SnapiDecimalType ignored ->
                  getFrameDescriptorBuilder()
                      .addSlot(FrameSlotKind.Object, getIdnName(entity), null);
              case SnapiBoolType ignored ->
                  getFrameDescriptorBuilder()
                      .addSlot(FrameSlotKind.Boolean, getIdnName(entity), null);
      ...
      case LetFun lf -> {
        ...
        ExpressionNode[] defaultArgs =
            JavaConverters.asJavaCollection(lf.p().ps()).stream()
                .map(p -> p.e().isDefined() ? recurseExp(p.e().get()) : null)
                .toArray(ExpressionNode[]::new);
      ...
      case LetFunRec lfr -> {
        ...
        ExpressionNode[] defaultArgs =
            JavaConverters.asJavaCollection(lfr.p().ps()).stream()
                .map(p -> p.e().isDefined() ? recurseExp(p.e().get()) : null)
                .toArray(ExpressionNode[]::new);
    ...
    return switch (in) {
      ...
      case BinaryExp be ->
          switch (be.binaryOp()) {
            ...
            case Neq ignored ->
                NotNodeGen.create(new EqNode(recurseExp(be.left()), recurseExp(be.right())));
      ...
      case IdnExp ie -> {
        ...
        yield switch (entity) {
          case MethodEntity b -> {
            ...
            yield slotLocation.depth() == 0
                ? ReadLocalVariableNodeGen.create(slotLocation.slot(), null)
                : ReadClosureVariableNodeGen.create(
                    slotLocation.depth(), slotLocation.slot(), null);
          }
          case LetBindEntity b -> {
            SlotLocation slotLocation = findSlot(b);
            yield slotLocation.depth() == 0
                ? ReadLocalVariableNodeGen.create(slotLocation.slot(), (SnapiType) tipe(b.b().e()))
                : ReadClosureVariableNodeGen.create(
                    slotLocation.depth(), slotLocation.slot(), (SnapiType) tipe(b.b().e()));
          }
          case LetFunEntity f -> {
            ...
            yield slotLocation.depth() == 0
                ...
                : ReadClosureVariableNodeGen.create(
                    slotLocation.depth(),
                    slotLocation.slot(),
                    (SnapiType) analyzer.idnType(f.f().i()));
          }
      ...
      case IfThenElse ite ->
          new IfThenElseNode(recurseExp(ite.e1()), recurseExp(ite.e2()), recurseExp(ite.e3()));
      case Proj proj -> RecordProjNodeGen.create(recurseExp(proj.e()), new StringNode(proj.i()));
      case Let let -> {
        StatementNode[] decls =
            JavaConverters.asJavaCollection(let.decls()).stream()
                .map(this::recurseLetDecl)
      ...
      case FunAbs fa -> {
        ...
        ExpressionNode[] defaultArgs =
            JavaConverters.asJavaCollection(fa.p().ps()).stream()
                .map(p -> p.e().isDefined() ? recurseExp(p.e().get()) : null)
                .toArray(ExpressionNode[]::new);
        // If the function has free variables it is a Closure
        if (hasFreeVars) {
      ...
      case FunApp fa when tipe(fa.f()) instanceof PackageEntryType -> {
        ...
        yield e.toTruffle(
            ...
            JavaConverters.asJavaCollection(fa.args()).stream()
                .map(a -> new SnapiArg(a.e(), tipe(a.e()), a.idn()))
                .toList(),
            this);
      }
      case FunApp fa -> {
