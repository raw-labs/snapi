...
import com.rawlabs.snapi.frontend.snapi.extensions.builtin.TypePackageBuilder
import com.rawlabs.snapi.frontend.snapi.source._
...
class SemanticAnalyzer(val tree: SourceTree.SourceTree)(implicit programContext: ProgramContext)
  ...
  override protected def errorDef: SourceNode ==> Seq[CompilerMessage] = {
    // Errors
    val snapiErrors: PartialFunction[SourceNode, Seq[CompilerMessage]] = {
    ...
    snapiErrors.orElse(super.errorDef)
  }
  ...
  override protected def nonErrorDef: SourceNode ==> Seq[CompilerMessage] = {
    ...
    snapiNonErrors.orElse(super.nonErrorDef)
  }
  ...
  private def repeatedOptionalArgument(fa: Exp, as: Vector[FunAppArg]): Boolean = {
    actualType(fa) match {
      case _: FunType =>
        // For calling Snapi functions, we check if arguments are unique.
...
//    case tree.parent.pair(e: Exp, _: FunParam) => out(e)
  ...
  override protected def actualTypeDef(n: Exp): Type = {
    val t = n match {
      case e: SnapiExp => actualTypeSnapiExp(e)
  ...
  private def actualTypeSnapiExp(n: SnapiExp): Type = n match {
  ...
  final private def getValue(report: CompatibilityReport, e: Exp): Either[ErrorCompilerMessage, SnapiValue] = {
    ...
    // Create an Snapi program with the expressions and declarations it depends on.
    ...
    // of Snapi, indirectly applied by calling Type.Cast, makes sure the cast is applied and hence we get back a
...
-                |Errors: $errs""".stripMargin)
  ...
  final override protected def expectedTypeDef(n: Exp): ExpectedType = {
    n match {
      case tree.parent.pair(e: Exp, parent: SnapiNode) => expectedTypeSnapi(e, parent)
      case _ => super.expectedTypeDef(n)
    }
  }

  private val snapibyte = SnapiByteType(Set(SnapiIsNullableTypeProperty(), SnapiIsTryableTypeProperty()))
  private val snapishort = SnapiShortType(Set(SnapiIsNullableTypeProperty(), SnapiIsTryableTypeProperty()))
  private val snapiint = SnapiIntType(Set(SnapiIsNullableTypeProperty(), SnapiIsTryableTypeProperty()))
  private val snapilong = SnapiLongType(Set(SnapiIsNullableTypeProperty(), SnapiIsTryableTypeProperty()))
  private val snapifloat = SnapiFloatType(Set(SnapiIsNullableTypeProperty(), SnapiIsTryableTypeProperty()))
  ...
  private val snapidate = SnapiDateType(Set(SnapiIsNullableTypeProperty(), SnapiIsTryableTypeProperty()))
  private val snapitimestamp = SnapiTimestampType(Set(SnapiIsNullableTypeProperty(), SnapiIsTryableTypeProperty()))
  private val snapinumbersAndString = OneOfType(snapinumerics.tipes :+ snapistring)
  private val snapitemporals = OneOfType(snapitime, snapiinterval, snapidate, snapitimestamp)
  private val snapinumericsTemporalsString = OneOfType(snapinumerics.tipes ++ snapitemporals.tipes :+ snapistring)
  private val snapinumericsTemporalsStringsBools =
    OneOfType(snapinumerics.tipes ++ snapitemporals.tipes ++ Vector(snapibool, snapistring))

  private def expectedTypeSnapi(e: Exp, parent: SnapiNode): ExpectedType = (parent: @unchecked) match {
    ...
    case UnaryExp(op, e1) =>
      assert(e eq e1)
      op match {
        case _: Not => snapibool
        case _: Neg => snapinumerics
      }
    case BinaryExp(op, e1, e2) =>
      ...
      op match {
        case _: Eq | _: Neq => expected(snapinumericsTemporalsStringsBools, None)
        case _: ComparableOp => expected(snapinumericsTemporalsString, None)
        case _: BooleanOp => expected(snapibool, Some(snapibool))
        case _: Plus => expected(snapinumbersAndString, None)
        case _ => expected(snapinumerics, Some(snapinumerics))
      }
