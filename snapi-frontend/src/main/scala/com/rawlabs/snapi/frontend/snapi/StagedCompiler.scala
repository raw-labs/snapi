 ...
 * This allows an Snapi/Snapi source to be evaluated in a new context, with a given environment and RawSettings.
 */
trait StagedCompiler {

  /**
   * Evaluate the given source code in a new context.
   * The source code is expected to be a valid Snapi/Snapi program.
  ...
  private def polyglotValueToSnapiValue(v: Value, t: Type)(implicit settings: RawSettings): SnapiValue = {
    t match {
      case t: SnapiTypeWithProperties if t.props.contains(SnapiIsTryableTypeProperty()) =>
        if (v.isException) {
          ...
          SnapiTryValue(Right(polyglotValueToSnapiValue(v, t.cloneAndRemoveProp(SnapiIsTryableTypeProperty()))))
        }
      case t: SnapiTypeWithProperties if t.props.contains(SnapiIsNullableTypeProperty()) =>
        if (v.isNull) {
          SnapiOptionValue(None)
        } else {
          SnapiOptionValue(Some(polyglotValueToSnapiValue(v, t.cloneAndRemoveProp(SnapiIsNullableTypeProperty()))))
        }
      case _: SnapiUndefinedType => throw new AssertionError("SnapiUndefined is not triable and is not nullable.")
      ...
      case SnapiRecordType(atts, _) =>
        val vs = atts.map(att => SnapiRecordAttr(att.idn, polyglotValueToSnapiValue(v.getMember(att.idn), att.tipe)))
        SnapiRecordValue(vs)
      case SnapiListType(innerType, _) =>
        ...
        for (i <- 0L until v.getArraySize) {
          ...
          seq.append(polyglotValueToSnapiValue(v1, innerType))
        }
        SnapiListValue(seq)
      ...
      case SnapiOrType(tipes, _) =>
        ...
        val tipe = tipes(idx)
        polyglotValueToSnapiValue(v1, tipe)
