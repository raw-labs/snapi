...
class TypeProtectCastEntry extends EntryExtension {
  ...
  override def returnType(
    ...
    // * when nullables are found, output type gets SnapiIsNullableProperty
    // * when tryables are found, output type gets SnapiIsTryableProperty
    //
  ...
  // cloned from snapi.SemanticAnalyzer
...
//    // TODO (msb): Refactor out the mergeType function of the Snapi SemanticAnalyzer and call it from here to do the
