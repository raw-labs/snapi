...
import com.rawlabs.snapi.frontend.snapi.extensions.builtin.IntervalPackageBuilder
import com.rawlabs.snapi.frontend.inferrer.api._
import com.rawlabs.snapi.frontend.snapi.extensions.builtin.{
  DatePackageBuilder,
  ListPackageBuilder,
...
trait SnapiTypeUtils {
  ...
  final def inferTypeToSnapiType(t: SourceType, makeNullable: Boolean, makeTryable: Boolean): Type = {
    ...
    t match {
      ...
      case SourceRecordType(atts, n) => wrapProps(
          SnapiRecordType(
            atts.map(att => SnapiAttrType(att.idn, inferTypeToSnapiType(att.tipe, makeNullable, makeTryable)))
          ),
          n || makeNullable,
          makeTryable
        )
      case SourceCollectionType(inner, n) => wrapProps(
          SnapiIterableType(inferTypeToSnapiType(inner, makeNullable, makeTryable)),
          n || makeNullable,
          makeTryable
        )
      ...
      case SourceOrType(ors) =>
        val options = ors
          .map(x => inferTypeToSnapiType(x, makeNullable, makeTryable)) // recurse with the flags
