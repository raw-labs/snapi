...
import com.rawlabs.snapi.frontend.snapi.antlr4.{CommentsAntlrSyntaxAnalyzer, NodeComments}
...
class SourceCommentsPrettyPrinter(maybeIndent: Option[Int] = None, maybeWidth: Option[Int] = None)
  ...
  override def snapiNode(n: SnapiNode): Doc = n match {
    ...
    case _ => super.snapiNode(n)
  }

  override def snapiTypeWithProperties(t: SnapiTypeWithProperties): Doc = t match {
    case FunType(ms, os, r, props) =>
      ...
      d
    case _ => super.snapiTypeWithProperties(t)
  }

  override def snapiExp(e: SnapiExp): Doc = e match {
    case Let(stmts, e) =>
