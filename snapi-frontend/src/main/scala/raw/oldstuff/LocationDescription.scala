...
trait LocationDescription {

  def url: String

  def options: Map[String, OptionValue]
...
final case class DropboxToken(accessToken: String, tokenType: String) extends LocationDescription {
  override def url: String = s"dropbox://$accessToken"
  override def options: Map[String, OptionValue] = Map.empty

  def this(url: String, options: Map[String, OptionValue]) = {
    this(options.get("accessToken").get.asInstanceOf[StringOptionValue].value, "Bearer")
  }
}
