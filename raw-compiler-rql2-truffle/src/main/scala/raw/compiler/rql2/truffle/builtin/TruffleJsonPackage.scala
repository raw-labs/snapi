      ...
      tipe match {
        ...
        case _ =>
          val result = tipe match {
            ...
            case Rql2ByteType(_) => new ByteParseJsonNode()
            case Rql2ShortType(_) => new ShortParseJsonNode()
            case Rql2IntType(_) => new IntParseJsonNode()
            case Rql2LongType(_) => new LongParseJsonNode()
            case Rql2FloatType(_) => new FloatParseJsonNode()
            case Rql2DoubleType(_) => new DoubleParseJsonNode()
            case Rql2DecimalType(_) => new DecimalParseJsonNode()
            case Rql2BoolType(_) => new BooleanParseJsonNode()
            case Rql2StringType(_) => new StringParseJsonNode()
            case Rql2DateType(_) => new DateParseJsonNode(dateFormat)
            case Rql2TimeType(_) => new TimeParseJsonNode(timeFormat)
            case Rql2TimestampType(_) => new TimestampParseJsonNode(timestampFormat)
            case Rql2IntervalType(_) => new IntervalParseJsonNode()
            case Rql2BinaryType(_) => new BinaryParseJsonNode()
