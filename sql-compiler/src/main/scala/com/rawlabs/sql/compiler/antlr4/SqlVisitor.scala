...
class SqlVisitor(
    ...
    private val errors: SqlVisitorParseErrors,
    var returnDescription: Option[String] = None
) extends PsqlParserBaseVisitor[SqlBaseNode] {

  private val positionsWrapper = new SqlPositions(positions, source)
  ...
  def sourceTextForContext(context: ParserRuleContext): Option[String] = {
    Option(context)
      .map(context => {
  ...
  override def visitCode(ctx: PsqlParser.CodeContext): SqlBaseNode = {
    Option(ctx)
      .map { context =>
  ...
  override def visitComment(ctx: PsqlParser.CommentContext): SqlBaseNode = {
    Option(ctx)
      .flatMap { context =>
  ...
  override def visitSingleLineComment(ctx: PsqlParser.SingleLineCommentContext): SqlBaseNode = {
    Option(ctx)
      .map { context =>
  ...
  override def visitSingleLineCommentEOF(ctx: PsqlParser.SingleLineCommentEOFContext): SqlBaseNode = {
    Option(ctx)
      .map { context =>
        val singleLineComment = SqlSingleLineCommentNode(visit(context.singleline_value_comment()))
  ...
  override def visitSingleParamComment(ctx: PsqlParser.SingleParamCommentContext): SqlBaseNode = {
    Option(ctx)
      .flatMap(context => Option(context.singleline_param_comment()).map(visit))
      .getOrElse(SqlErrorNode())
  }

  override def visitSingleTypeComment(ctx: PsqlParser.SingleTypeCommentContext): SqlBaseNode = {
    Option(ctx)
      .flatMap(context => Option(context.singleline_type_comment()).map(visit))
      .getOrElse(SqlErrorNode())
  }

  override def visitSingleReturnComment(ctx: PsqlParser.SingleReturnCommentContext): SqlBaseNode = {
    Option(ctx)
      .flatMap(context => Option(context.singleline_return_comment()).map(visit))
      .getOrElse(SqlErrorNode())
  }

  override def visitSingleDefaultComment(ctx: PsqlParser.SingleDefaultCommentContext): SqlBaseNode = {
    Option(ctx)
  ...
  override def visitSingleNormalComment(ctx: PsqlParser.SingleNormalCommentContext): SqlBaseNode = {
    Option(ctx)
      .flatMap(context => Option(context.singleline_normal_comment_value()).map(visit))
      .getOrElse(SqlErrorNode())
  }

  override def visitSingleline_param_comment(ctx: PsqlParser.Singleline_param_commentContext): SqlBaseNode = {
    Option(ctx)
      .map(context => {
  ...
  override def visitSingleline_type_comment(ctx: PsqlParser.Singleline_type_commentContext): SqlBaseNode = {
    Option(ctx)
      .map(context => {
        if (context.SL_WORD(0) == null) {
  ...
  override def visitSingleline_default_comment(ctx: PsqlParser.Singleline_default_commentContext): SqlBaseNode = {
    Option(
      ctx
    )
      .map(context => {
          ...
          params.get(name) match {
            case Some(value) =>
              ...
              if (existsParamDefault) addError(s"Duplicate parameter default definition for $name", context)
                ...
                .update(name, value.copy(default = Some(defaultValue), nodes = value.nodes :+ paramDefaultComment))
  ...
  override def visitSingleline_return_comment(ctx: PsqlParser.Singleline_return_commentContext): SqlBaseNode = {
    Option(
      ctx
    )
      .map(context => {
  ...
  override def visitMultiline_comment(ctx: PsqlParser.Multiline_commentContext): SqlBaseNode = {
    Option(ctx)
      .map { context =>
  ...
  override def visitMultilineParamComment(ctx: PsqlParser.MultilineParamCommentContext): SqlBaseNode = {
    Option(ctx)
      .flatMap(context => Option(context.multiline_param_comment()).map(visit))
      .getOrElse(SqlErrorNode())
  }

  override def visitMultilineTypeComment(ctx: PsqlParser.MultilineTypeCommentContext): SqlBaseNode = {
    Option(ctx)
      .flatMap(context => Option(context.multiline_type_comment()).map(visit))
      .getOrElse(SqlErrorNode())
  }

  override def visitMultilineDefaultComment(ctx: PsqlParser.MultilineDefaultCommentContext): SqlBaseNode = {
    Option(ctx)
      .flatMap(context => Option(context.multiline_default_comment()).map(visit))
      .getOrElse(SqlErrorNode())
  }

  override def visitMultilineReturnComment(ctx: PsqlParser.MultilineReturnCommentContext): SqlBaseNode = {
    Option(ctx)
  ...
  override def visitMultilineNormalComment(ctx: PsqlParser.MultilineNormalCommentContext): SqlBaseNode = {
    Option(ctx)
      .flatMap(context => Option(context.multiline_normal_comment_value()).map(visit))
      .getOrElse(SqlErrorNode())
  }

  override def visitMultiline_param_comment(ctx: PsqlParser.Multiline_param_commentContext): SqlBaseNode = {
    Option(ctx)
      .map(context => {
  ...
  override def visitMultiline_type_comment(ctx: PsqlParser.Multiline_type_commentContext): SqlBaseNode = {
    Option(ctx)
      .map(context => {
        if (context.multiline_word_or_star(0) == null) {
          addError("Missing name for syntax @type <name> <type>", context)
  ...
  override def visitMultiline_default_comment(ctx: PsqlParser.Multiline_default_commentContext): SqlBaseNode = {
    Option(
      ctx
    )
      .map(context => {
  ...
  override def visitMultiline_return_comment(ctx: PsqlParser.Multiline_return_commentContext): SqlBaseNode = {
    Option(
      ctx
    )
      .map(context => {
  ...
  override def visitMultiline_normal_comment_value(
      ctx: PsqlParser.Multiline_normal_comment_valueContext
  ): SqlBaseNode = {
    Option(
      ctx
    )
  ...
  override def visitStmtItems(ctx: PsqlParser.StmtItemsContext): SqlBaseNode = {
    Option(ctx)
      .map { context =>
  ...
  override def visitParenStmt(ctx: PsqlParser.ParenStmtContext): SqlBaseNode = {
    Option(ctx)
      .flatMap(context => Option(context.stmt()).map(visit))
      .getOrElse(SqlErrorNode())
  }

  override def visitParenStmtSqureBr(ctx: PsqlParser.ParenStmtSqureBrContext): SqlBaseNode = {
    Option(ctx)
      .flatMap(context => Option(context.stmt()).map(visit))
      .getOrElse(SqlErrorNode())
  }

  override def visitNestedStmtSqureBr(ctx: PsqlParser.NestedStmtSqureBrContext): SqlBaseNode = {
    Option(ctx)
      .flatMap(context => Option(context.stmt()).map(visit))
      .getOrElse(SqlErrorNode())
  }

  override def visitFunCallStmt(ctx: PsqlParser.FunCallStmtContext): SqlBaseNode = {
    Option(ctx)
      .map { context =>
  ...
  override def visitUnknownStmt(ctx: PsqlParser.UnknownStmtContext): SqlBaseNode = {
    Option(ctx)
      .map { context =>
  ...
  override def visitNestedStmt(ctx: PsqlParser.NestedStmtContext): SqlBaseNode = {
    Option(ctx)
      .flatMap(context => Option(context.stmt()).map(visit))
      .getOrElse(SqlErrorNode())
  }
...
//    .getOrElse(SqlErrorNode())
  ...
  override def visitProjStmt(ctx: PsqlParser.ProjStmtContext): SqlBaseNode = {
    Option(ctx)
      .flatMap(context => Option(context.proj()).map(visit))
      .getOrElse(SqlErrorNode())
  }

  override def visitLiteralStmt(ctx: PsqlParser.LiteralStmtContext): SqlBaseNode = {
    Option(ctx)
      .flatMap(context => Option(context.literal()).map(visit))
      .getOrElse(SqlErrorNode())
  }

  override def visitKeywordStmt(ctx: PsqlParser.KeywordStmtContext): SqlBaseNode = {
    Option(ctx)
...
//    .getOrElse(SqlErrorNode())
  ...
  override def visitIdntStmt(ctx: PsqlParser.IdntStmtContext): SqlBaseNode = {
    Option(ctx)
      .flatMap(context => Option(context.idnt()).map(visit))
      .getOrElse(SqlErrorNode())
  }

  override def visitParamStmt(ctx: PsqlParser.ParamStmtContext): SqlBaseNode = {
    Option(ctx)
      .map(context => {
...
//    .getOrElse(SqlErrorNode())
  ...
  override def visitReserved_keyword(ctx: PsqlParser.Reserved_keywordContext): SqlBaseNode = {
    Option(ctx)
      .map { context =>
        val keyword = SqlKeywordNode(context.getText)
        positionsWrapper.setPosition(ctx, keyword)
  ...
  override def visitOperator(ctx: PsqlParser.OperatorContext): SqlBaseNode = {
    Option(ctx)
      .map { context =>
  ...
  override def visitOperatorStmt(ctx: PsqlParser.OperatorStmtContext): SqlBaseNode = {
    Option(ctx)
      .flatMap(context => Option(context.operator()).map(visit))
      .getOrElse(SqlErrorNode())
  }

  override def visitProperProj(ctx: PsqlParser.ProperProjContext): SqlBaseNode = {
    Option(ctx)
      .map { context =>
  ...
  override def visitMissingIdenProj(ctx: PsqlParser.MissingIdenProjContext): SqlBaseNode = {
    Option(ctx)
      .map { context =>
  ...
  override def visitIdnt(ctx: PsqlParser.IdntContext): SqlBaseNode = {
    Option(ctx)
      .map { context =>
  ...
  override def visitStringLiteral(ctx: PsqlParser.StringLiteralContext): SqlBaseNode = {
    Option(ctx)
      .map { context =>
        val value = context.getText
  ...
  override def visitIntegerLiteral(ctx: PsqlParser.IntegerLiteralContext): SqlBaseNode = {
    Option(ctx)
      .map { context =>
        val value = context.getText
  ...
  override def visitFloatingPointLiteral(ctx: PsqlParser.FloatingPointLiteralContext): SqlBaseNode = {
    Option(ctx)
      .map { context =>
        val value = context.getText
  ...
  override def visitBooleanLiteral(ctx: PsqlParser.BooleanLiteralContext): SqlBaseNode = {
    Option(ctx)
      .map { context =>
        val value = context.getText
  ...
  override def visitTipe(ctx: PsqlParser.TipeContext): SqlBaseNode = {
    Option(ctx)
      .map { context =>
  ...
  override def visitCommentStmt(ctx: PsqlParser.CommentStmtContext): SqlBaseNode = {
    super.visitCommentStmt(ctx)
  }

  override def visitTypeStmt(ctx: PsqlParser.TypeStmtContext): SqlBaseNode = {
    super.visitTypeStmt(ctx)
  }
  ...
  override def visitMultiline_unknown_type_comment(
      ctx: PsqlParser.Multiline_unknown_type_commentContext
  ): SqlBaseNode = {
    throw new AssertionError(assertionMessage)
  }
