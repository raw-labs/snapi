...
import raw.compiler.{AiValidateLSPRequest, ErrorLSPResponse, ErrorMessage, ProgramEnvironment, ValidateLSPRequest}
...
trait LspValidateTest extends CompilerTestContext {
  ...
  test("validate simple code test") { _ =>
    ...
    val response = doLsp(ValidateLSPRequest(code, queryEnvironment))
    response match {
  ...
  test("AI validate package that does not exist") { _ =>
    val code = """let
      ...
      |in
      |    data""".stripMargin
    val response = doLsp(AiValidateLSPRequest(code, queryEnvironment))
    response match {
