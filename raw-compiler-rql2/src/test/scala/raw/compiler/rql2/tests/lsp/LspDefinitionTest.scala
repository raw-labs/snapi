...
import raw.compiler.{DefinitionLSPRequest, DefinitionLSPResponse, ErrorLSPResponse, Pos, ProgramEnvironment}
...
trait LspDefinitionTest extends CompilerTestContext {
  ...
  test("go to definition identifier at usage test") { _ =>
    val code = """let
      ...
      |a
      |""".stripMargin
    val response = doLsp(DefinitionLSPRequest(code, queryEnvironment, Pos(4, 1)))
    response match {
  ...
  test("got to definition identifier at definition test") { _ =>
    val code = """let
      ...
      |a
      |""".stripMargin
    val response = doLsp(DefinitionLSPRequest(code, queryEnvironment, Pos(2, 1)))
    response match {
      case DefinitionLSPResponse(Pos(line, column), _) =>
        assertResult(2)(line)
        assertResult(1)(column)
  ...
  test("go to definition function identifier recursive function at usage test") { _ =>
    val code = """let
      ...
      |in
      |b(2)""".stripMargin
    val response = doLsp(DefinitionLSPRequest(code, queryEnvironment, Pos(5, 1)))
    response match {
  ...
  test("go to definition let function entity recursive function at definition test") { _ =>
    val code = """let
      ...
      |in
      |b(2)""".stripMargin
    val response = doLsp(DefinitionLSPRequest(code, queryEnvironment, Pos(2, 7)))
    response match {
      case DefinitionLSPResponse(Pos(line, char), _) =>
        assertResult(2)(line)
        assertResult(7)(char)
  ...
  test("go to definition function identifier at definition test") { _ =>
    ...
    val response = doLsp(DefinitionLSPRequest(code, queryEnvironment, Pos(2, 3)))
    response match {
      case DefinitionLSPResponse(Pos(line, char), _) =>
        assertResult(2)(line)
        assertResult(1)(char)
  ...
  test("go to definition function identifier at usage test") { _ =>
    val code = """let
      ...
      |in
      |    let bbb = buildCollection(5), ttt = Collection.Build(1,2,3) in Collection.Filter(ttt, t -> t > 1 )""".stripMargin
    val response = doLsp(DefinitionLSPRequest(code, queryEnvironment, Pos(8, 15)))
    response match {
      case DefinitionLSPResponse(Pos(line, char), _) =>
        assertResult(2)(line)
        assertResult(1)(char)
  ...
  test("go to definition function parameter test") { _ =>
    val code = """let
      ...
      |in
      |    let bbb = buildCollection(5), ttt = Collection.Build(1,2,3) in Collection.Filter(ttt, t -> t > 1 )""".stripMargin
    val response = doLsp(DefinitionLSPRequest(code, queryEnvironment, Pos(2, 20)))
    response match {
      case DefinitionLSPResponse(Pos(line, char), _) =>
        assertResult(2)(line)
  ...
  test("go to definition function parameter usage test") { _ =>
    val code = """let
      ...
      |in
      |    let bbb = buildCollection(5), ttt = Collection.Build(1,2,3) in Collection.Filter(ttt, t -> t > 1 )""".stripMargin
    val response = doLsp(DefinitionLSPRequest(code, queryEnvironment, Pos(3, 29)))
    response match {
      case DefinitionLSPResponse(Pos(line, char), _) =>
        assertResult(2)(line)
        assertResult(19)(char)
  ...
  test("go to definition let should return empty response") { _ =>
    val code = """let
      ...
      |in
      |    let bbb = buildCollection(5), ttt = Collection.Build(1,2,3) in Collection.Filter(ttt, t -> t > 1 )""".stripMargin
    val response = doLsp(DefinitionLSPRequest(code, queryEnvironment, Pos(1, 1)))
  ...
  test("go to definition field of a collection") { _ =>
    ...
    val response = doLsp(DefinitionLSPRequest(code, queryEnvironment, Pos(4, 32)))
    response match {
      case DefinitionLSPResponse(Pos(line, char), _) =>
        assertResult(2)(line)
  ...
  test("go to definition field of a collection nested") { _ =>
    val code = """let
      ...
      |in
      |Collection.Filter(data, d -> d.aaaaaaaaaaaa.cccccccccccc > 0)""".stripMargin
    val response = doLsp(DefinitionLSPRequest(code, queryEnvironment, Pos(4, 45)))
    response match {
      case DefinitionLSPResponse(Pos(line, char), _) =>
        assertResult(2)(line)
