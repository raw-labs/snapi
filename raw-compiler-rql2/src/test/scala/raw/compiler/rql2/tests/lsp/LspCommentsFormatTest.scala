  ...
  test("snapi example commented") { _ =>
    ...
    assertFormattedCode(
      ...
      """// Machine information taken from a postgresql database
        ...
        |    ),
        |    lastErrors = Collection.Filter(errors(machineId), (x) -> x.timestamp < lastFailure and x.timestamp > startMeasure)
        |  in
  ...
  test("RD-9333") { _ =>
    ...
    assertFormattedCode(
      ...
      """// A general function for prompts to OpenAI
        ...
        |            bodyString = "{\"model\": \"" + model + "\", \"messages\": [{\"role\": \"user\", \"content\": \"" + messages +
        |              "\"}], \"temperature\": " +
        |              String.From(temperature) +
        |              "}"
        |          )
        ...
        |              f = Collection.Zip(titles, indexes),
        |              titlesText = Collection.Transform(f, (r) -> "##" + String.Replicate("#", Int.From(r._2)) + " " + r._1),
        ...
        |                "gpt-3.5-turbo",
        |                0,
        |                "Given the content (in markdown): ```" + String.Replace(Json.Print(markdownBody), "\"", "") +
        |                  "```, describe it in plain english. Do not mention the markdown structure at all."
        |              )
        |            in
