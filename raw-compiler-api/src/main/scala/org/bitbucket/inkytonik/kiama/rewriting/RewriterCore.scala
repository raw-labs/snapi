...
//  */
//trait RewriterCore {
//  // MONKEY PATCHs: (ns) Rewrite the methods that are the most used by RAW to usie iteration instead of
//  // functional iteration with foreach/closures. This avoids raising a NonLocalControlReturn exception to
//  // implement the return and removes 8 to 10 layers of calls from the stack (the ones introduced by foldLeft())
//
...
//            sys.error(
//              s"""dup illegal arguments: $ctor got (${children.mkString(",")})
