...
class BinaryExpGeTest extends Rql2TruffleCompilerTestContext with CombinationSpecTestHelper {

  test("1 >= 1")(it => it should evaluateTo("true"))
  test("1 >= 2")(it => it should evaluateTo("false"))
  test("2 >= 1")(it => it should evaluateTo("true"))

  test("true >= false")(it => it should typeErrorAs("expected either number or temporal or string but got bool"))
  ...
  test("date timestamp >= date timestamp") { _ =>
    forAll(combinations(dateTimestamp, dateTimestamp)) {
      case (n1, n2) =>
        TestData(s"${n1.v1} >= ${n2.v1}") should evaluateTo("true")
        TestData(n1.v2 + " >= " + n2.v1) should evaluateTo("true")
        TestData(s"${n1.v1} >= ${n2.v2}") should evaluateTo("false")
    }
  }
